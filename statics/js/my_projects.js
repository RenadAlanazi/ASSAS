...
const renderTablePaginated = () => {
  ...
  // ✅ ADD THESE BACK (you accidentally removed them)
...
const applyFiltersAndSearch = () => {
  ...
  filteredReports = allReports.filter(report => {
    ...
    if (dateFromVal && report.created_at &&
      new Date(report.created_at.toDate?.() || report.created_at) < dateFromVal) match = false;

    if (dateToVal && report.created_at &&
      new Date(report.created_at.toDate?.() || report.created_at) > dateToVal) match = false;
